 ...
 *  - a minimized window is not offered by the desktop capturer at all — it is absent from the
...
test('shows a minimized window to capture it, and minimizes it again afterwards', async () => {
  ...
  const deps: CaptureDependencies = {
    sessions: async () => [session('a', 11)],
    // Absent while minimized, present once shown — which is the measured behaviour.
    getSources: async () => {
      enumerated += 1;
