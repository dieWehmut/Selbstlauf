...
設定頁的「關於」分區會列出型錄中每個 agent CLI 的目前版本與 npm 上的最新版本，
並標記落後的工具。每張卡片的「升級」按鈕會呼叫本機
`POST /api/environment/upgrade`（請求內容 `{ "id": "claude" }`），
頁首的「全部升級」呼叫 `POST /api/environment/upgrade-all`，只升級報告中標記為
落後且已安裝的工具，不會順手安裝機器上原本沒有的軟體。

服務端只接受型錄中的工具 id，指令由型錄條目組裝，呼叫方無法夾帶任意套件名稱或
多餘的 npm 參數；全域安裝一次只執行一個，避免並行寫壞同一個 prefix；安裝完成後
會重新掃描環境，讓面板顯示實際安裝的版本。安裝結果會寫入稽核記錄，但不記錄
token 或路徑。Pages 展示站以記憶體樣本模擬相同按鈕，不會安裝任何東西。

### 外觀設定

設定頁側欄的「外觀」分區提供外觀選擇：三張主題預覽卡分別對應系統、淺色與深色，
下方的程式碼對照條顯示目前主題與修改後的 surface、accent 與 contrast 取值。
卡片下方是逐列的設定清單：強調色下拉選單、背景與前景取色器、UI 字型與字重、
內容字型（可選「與介面字型相同」）、半透明側邊欄開關，以及對比度滑桿。

對比度決定面板、邊框與頁面背景的距離：數值越高，表面越清楚地浮出背景；
半透明側邊欄讓側欄透出頁面背景，整體讀作一個表面。自訂以配色方案為單位儲存在
瀏覽器中，因此自訂過的深色配色不會在切換到淺色再切回時遺失；「恢復預設」會把
目前方案還原成內建配色。這些變更都是純前端行為，不會寫入服務端狀態。

### 設定分區

設定頁改用左側分類導覽取代原本的分區標籤列：頂端是「返回應用」與「搜尋設定」
（依名稱即時過濾分區），下方依「個人」與「整合」兩組列出全部分區。

- **個人**：一般、通知、匯入、個人資料、外觀、家長控制、Trusted contact、語音、
  設定、個人化、寵物、鍵盤快速鍵、使用情況與計費、帳戶。
- **整合**：電腦操控、應用快照、外掛、瀏覽器。

其中「使用情況與計費」「帳戶」「應用快照」的數值都由目前工作階段與環境報告即時
計算，不顯示任何佔位數字；「電腦操控」的「允許開啟執行位置」會真正停用程序表中
對應的按鈕；「個人資料」的顯示名稱會出現在側欄品牌位置；「通知」的
「保留最近事件筆數」會真正限制事件時間軸保留的筆數。啟用「家長控制」後，儲存
設定需要再次輸入 PIN；該 PIN 僅在本機混淆儲存，不是安全邊界。

### 視窗標題列與系統匣

桌面版使用自繪標題列搭配原生視窗按鈕：標題列左側是側欄開關、上一頁、下一頁，
接著是「檔案 / 編輯 / 檢視 / 說明」四個可展開的選單，右側保留系統的最小化、
最大化與關閉按鈕。第二列仍是原本的頁面標題與操作區。

關閉視窗不會結束應用程式，也不會停止 watchdog 服務，而是隱藏到系統匣。
**只有系統匣選單中的「結束」會真正結束程式**：

- 左鍵點擊系統匣圖示：顯示並聚焦主視窗（最小化時先還原）。
- 右鍵選單：開啟主介面、設定（直接進入設定頁）、開機自動啟動（核取方塊，
  狀態取自服務端回報值）、關於 Selbstlauf（直接進入「帳戶」分區）、結束。

在沒有系統匣的機器上視窗會正常關閉，避免程式變成既無視窗又無系統匣的隱藏
行程。啟用「開機自動啟動」會在安裝目錄註冊目前使用者的登入工作。

### 圖示資源

所有圖示（安裝程式、桌面與開始功能表捷徑、視窗與工作列、WebUI favicon 與側欄
品牌圖示）都由同一張來源圖片產生：
...
指令碼只使用內建 GDI+，從 `assets/selbstlauf-icon-source.png` 產生 7 幀 ICO
（16/24/32/48/64/128/256）與各尺寸 PNG，重複執行結果一致，可用 `-Source`
指定其他來源圖片。
