...
- **个人**：常规、通知、导入、个人资料、外观、家长控制、信任联系人、语音、
  配置、个性化、宠物、键盘快捷键、使用统计、账户。
- **集成**：电脑操控、应用快照、插件、浏览器。

其中“使用统计”“账户”“应用快照”的数值都由当前会话与环境报告实时算出，
...
侧栏顶部直接是“进程 / 事件 / 设置”三个主导航，**下面是发现到的进程列表**：按**运行
位置分组**（Tabby、Visual Studio Code、dsh-shortcut 等，即主表格“运行位置”一列显示的
宿主），每组一个小标题；**点选某一行会在主区域打开该进程的详情页**，侧栏对应行整行
高亮。列表有独立的搜索框与独立滚动区，进程再多也不会把主导航和底部条挤出视野，也不会
撑宽侧栏。
...
能不能续写它的依据，所以放在行上而不是藏在一次点击之后。每行三行信息：

```
Codex                                    34m 10s
Goal · complete · 01a0bd1e-19cf-7601…
Tabby · PID 30780
```
