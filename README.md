...
侧栏顶部是品牌位，下面是“进程 / 事件 / 设置”三个主导航，**再下面是发现到的进程
列表**：按**运行位置分组**（Tabby、Visual Studio Code、dsh-shortcut 等，即主表格
“运行位置”一列显示的宿主），每组一个小标题；**点选某一行会在主区域打开该进程的
详情页**，侧栏对应行整行高亮。列表有独立的搜索框与独立滚动区，进程再多也不会把
主导航和底部条挤出视野，也不会撑宽侧栏。
...
侧栏**底部条点击后向上弹出菜单**（主题切换、设置、收起侧栏）：向下弹会掉出窗口
底部。Escape 与点击外部都能关闭，与标题栏下拉菜单行为一致。
