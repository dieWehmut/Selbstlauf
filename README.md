...
| 进程没有自己的窗口 | 不适用：DeepSeek Harness 是网页界面，没有可抓的窗口 |
| 多个进程共用一个窗口 | 画面是**整个窗口**，面板会写明“此窗口内有 N 个受监控进程” |

因此面板**如实说明当前是哪种情况**，而不是给一个空白框：最小化时提示“窗口已最小化，无法
抓取画面。还原该窗口后点刷新即可查看”。一次抓取会枚举并截取机器上**所有**窗口、耗时约
300ms 且**不随窗口数量增长**，所以它**按需抓取**（切换进程时抓一次，另有刷新按钮），不是
定时轮询。
