...
if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) {
    $candidate = Join-Path $PSScriptRoot 'scripts\install\windows\install-codex-windows.ps1'
...
if ([string]::IsNullOrWhiteSpace($coreText)) {
    throw "Downloaded Selbstlauf core from '$coreUrl' was empty."
}
...
if ($null -eq (Get-Command Install-AiCliBypass -CommandType Function -ErrorAction SilentlyContinue)) {
    throw 'The Selbstlauf core did not define Install-AiCliBypass.'
}
